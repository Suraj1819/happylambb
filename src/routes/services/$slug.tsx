...
// ============== VIDEO GALLERY WITH YOUTUBE THUMBNAILS ==============
...
function PhotoGallery({
  ...
  const handleShowLess = () => {
    setLimit(6); // Reset to initial 6 images
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 🟢 LOAD MORE / SHOW LESS BUTTON WITH ARROW */}
      {totalImages > 6 && (
...
function ServicePage() {
  ...
  const platforms = [
    { icon: Youtube, label: "YouTube", color: "text-foreground" },
    { icon: Instagram, label: "Instagram", color: "text-foreground" },
    { icon: Facebook, label: "Facebook", color: "text-foreground" },
    { icon: Tv, label: "Broadcast", color: "text-foreground" },
  ];
  ...
  return (
    <>
      <section className="relative pt-32 pb-16 bg-background dark:bg-zinc-950 border-b border-border/40 dark:border-zinc-800">
        <div className="mx-auto max-w-[1400px] px-6 sm:px-10 w-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <Reveal delay={0.2}>
              <HeroCarousel images={carouselImages} title={service.title} />
            </Reveal>
          </div>
        </div>
      </section>
          ...
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {features.map((feature, i) => (
              <Reveal key={i} delay={i * 0.06}>
                <div className="group rounded-xl border border-border/40 dark:border-zinc-800 bg-surface/30 dark:bg-zinc-900/50 p-6 text-center transition-all hover:border-border/60 dark:hover:border-zinc-600">
      ...
      {/* 🟢 DYNAMIC MEDIA GALLERY WITH LOAD MORE */}
        ...
        <section className="py-24 bg-background dark:bg-zinc-950">
          <div className="mx-auto max-w-[1400px] px-6 sm:px-10">
            <Reveal className="text-center max-w-3xl mx-auto">
              ...
              <h2 className="text-4xl md:text-5xl font-medium tracking-tight text-foreground dark:text-white">
                Our <br />
                <span className="italic text-muted-foreground/60 dark:text-zinc-400">Work.</span>
              </h2>
            </Reveal>
          ...
          <div className="mt-12 space-y-4">
            {service.faqs.map((f, i) => (
              <Reveal key={f.q} delay={i * 0.05}>
                <details className="group rounded-xl border border-border/40 dark:border-zinc-800 bg-surface/30 dark:bg-zinc-900/50 transition-all hover:border-border/60 dark:hover:border-zinc-600">
                  <summary className="flex cursor-pointer items-center justify-between p-5 font-medium text-sm tracking-tight transition-colors marker:content-none hover:text-foreground dark:hover:text-white">
                    <span>{f.q}</span>
      ...
      <section className="bg-ink py-32 text-ink-foreground relative overflow-hidden text-center border-t border-ink-foreground/10 dark:border-zinc-800">
      ...
      {lightboxOpen && (
        <Lightbox items={allMedia} initialIndex={lightboxIndex} onClose={closeLightbox} />
      )}
    </>
  );
}
