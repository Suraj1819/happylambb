...
function StudioCarousel() {
  ...
  return (
    <div className="relative w-full h-[450px] sm:h-[550px] rounded-2xl overflow-hidden border border-border/40 dark:border-zinc-800 shadow-xl">
      <AnimatePresence>
        <motion.div
          key={current}
          initial={{ opacity: 0 }}
...
function Index() {
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <Reveal>
              ...
              <div className="mt-8 flex flex-wrap items-center gap-6">
                <Link
                  to="/work"
                  className="inline-flex items-center gap-2 border-b border-foreground/40 dark:border-zinc-500 pb-1 text-sm tracking-wider hover:gap-4 transition-all duration-300"
                >
      ...
      <section className="border-y border-border/30 dark:border-zinc-800 bg-surface/30 dark:bg-zinc-900/50 py-24">
        <div className="mx-auto max-w-[1400px] px-6 sm:px-10">
          ...
          {featuredMain && (
            <Reveal className="mt-14">
              <VideoPlayer
                ...
                aspect="16/9"
                className="rounded-xl shadow-sm border border-border/40 dark:border-zinc-800"
              />
            </Reveal>
          )}
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {whyChooseUs.map((item, i) => (
              <Reveal key={item.title} delay={(i % 3) * 0.05}>
                {/* 🟢 FIX: 3D lift hata diya */}
                <div className="group relative h-full overflow-hidden rounded-xl border border-border/40 dark:border-zinc-800 bg-surface/30 dark:bg-zinc-900/50 p-7 transition-all duration-300 hover:border-border/60 dark:hover:border-zinc-600">
      ...
      {/* ═══════════════ 8. FINAL SYMMETRIC DIVIDER ═══════════════ */}
      <section className="py-24 bg-background dark:bg-zinc-950 border-t border-border/30 dark:border-zinc-800">
