    ...
    <div className="relative mx-auto max-w-4xl">
      <div className="relative min-h-[19rem] sm:min-h-[16rem]">
        <AnimatePresence mode="wait">
          ...
          >
            {/* 🟢 FIX 1: Stars ko Orange se Black/Gray kar diya */}
            <div className="flex gap-1 text-foreground/20">
              {Array.from({ length: 5 }).map((_, s) => (
                <Star key={s} className="h-4 w-4 fill-foreground/80" />
              ))}
            </div>
      ...
      {/* 🟢 FIX 2: Navigation dots ko Orange se Gray kar diya */}
        ...
        {TESTIMONIALS.map((_, d) => (
          <button
            ...
            className={`h-1.5 rounded-full transition-all duration-300 ${
              d === i ? "w-10 bg-border/80" : "w-4 bg-border/40 hover:bg-border/60"
            }`}
          />
        ))}
      </div>
